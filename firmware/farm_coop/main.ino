...
int ow = D2;    // put the onewire bus on D0
...
void setup(void) {
  ...
  Spark.publish("farm_coop", "online_1.2");
}
...
//   Serial.println(" Fahrenheit");
  ...
  if ((now - lastPublish) >= 60000) {
    Spark.publish("farm/coop/temp", String(fahrenheit));
    lastPublish = now;
    publishCount++;
  }
  ...
  // 2 * 60s == 2 minutes on
  if (publishCount >= 2) {
      System.sleep(SLEEP_MODE_DEEP, 15 * 60);    //sleep for 15 minutes
  }
}
